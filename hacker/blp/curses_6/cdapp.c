...
static char current_cat[MAX_STRING] = "\0";

const char *title_file = "title.cdb";
const char *tracks_file = "tracks.adb";
    ...
    do {
        ...
        switch (choice) {
            ...
            case 'u':
                ...
                break;
            default:
                fprintf(stderr, "sorry, you have input a wrong choice\n");
                break;
        }
    } while(choice != 'q');
...
void draw_menu(char *options[], int current_highlight, int start_row, int start_col)
{
    ...
    mvprintw(start_row + current_row + 3, start_col, "Move highlight then presss Return ");
    refresh();
}
...
void count_cds()
{
    ...
    mvprintw(ERROR_LINE, 0, "Database contains %d title, with a total of %d tracks.", titles, tracks);
    get_return();
}
